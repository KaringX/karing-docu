...
### Приложения для Android/iOS без сети/видео не воспроизводятся (Новости NetEase)

- Проверьте настройки - TUN - «Подключить HTTP-прокси к VPN»

- Отключите «Подключить HTTP-прокси к VPN» и попробуйте снова

- Примечание: Включение «Подключить HTTP-прокси к VPN» настраивает системный прокси (HTTP). Некоторые приложения автоматически подключаются к системному прокси после его обнаружения. Однако эти приложения могут использовать SOCKS-прокси для подключения к системному прокси, который не поддерживается системным прокси, что может вызывать проблемы с сетью для некоторых приложений. Также обратите внимание, что для работы некоторых приложений требуется включение «Подключить HTTP-прокси к VPN». Пожалуйста, включайте/отключайте эту функцию в соответствии с вашими потребностями.
...
### Windows: после включения режима TUN в karingservice.exe потребление памяти и CPU очень высокое

- Вызвано конфликтами в режиме TUN, например, с VPN-сервисами серии Clash. Пожалуйста, удалите другое VPN-программное обеспечение и перезапустите Karing.

- Вызвано проблемами с маршрутизацией, например, когда включена переадресация данных Windows (превращая Windows в маршрутизатор).

Команда запроса (PowerShell):

```
Get-NetIPInterface | Where-Object {$_.Forwarding -eq 'Enabled'}

````
Пожалуйста, отключите эту функцию. Справочник команд (PowerShell):

```
Set-NetIPInterface -ifAlias ​​​​<имя вашей сетевой карты> -Forwarding Disabled

```
- Вызвано включением точек доступа. Отключите точки доступа или отключите «автоматическую маршрутизацию» в TUN.
