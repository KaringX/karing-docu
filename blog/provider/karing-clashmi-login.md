...
### WebView + 隐藏订阅地址 + X-HWID

在服务商支持的前提下，WebView 登录还可以与隐藏订阅地址、`X-HWID` 组合使用：账号密码仍只提交给服务商，订阅地址不在 APP 常规界面展示；服务商可以把 HWID 与用户关联，限制设备数量，并提供设备查看、删除或拒绝能力。这样，仅复制订阅地址而没有有效 HWID，通常无法继续获取订阅更新。

这些约束必须由服务商落实。订阅接口需要读取并校验 `X-HWID`，维护用户与设备的映射，在每次订阅请求时执行设备限制，并提供设备管理和撤销能力。APP 仅上报 `X-HWID` Header，并不会自动产生服务端约束。

这套组合也有明确的安全边界：隐藏订阅地址是产品界面层的保护，并非密码学隐藏；`X-HWID` 是客户端提供的设备标识，不等同于设备私钥的持有证明，也可能被复制或伪造。删除设备主要阻止该设备后续获取或更新订阅，不保证已经下载的节点凭据立即失效；若要实现这一效果，还需要节点凭据轮换、过期机制或节点侧设备认证配合。

| 对比维度 | WebView + 隐藏订阅地址 + X-HWID | OAuth + PKCE + Ed25519 |
| --- | --- | --- |
| 服务商改造量 | 较小，主要改造订阅接口和设备管理 | 较大，需要完整授权与设备密钥协议 |
| 设备绑定 | 通过服务端记录用户与 HWID 的关系 | 通过用户授权与设备公钥绑定 |
| 设备撤销 | 删除或拒绝 HWID，阻止后续订阅请求 | 撤销设备密钥或授权，阻止后续协议请求 |
| 订阅地址保护 | 常规界面不展示，并要求请求携带有效 HWID | 可由协议网关直接返回配置，不暴露真实订阅地址 |
| 认证强度 | 客户端设备标识，可被复制或伪造 | 私钥签名与 challenge 可提供密码学持有证明 |
| 维护复杂度 | 较低，但仍需维护面板适配和 HWID 服务端逻辑 | 较高，需要维护授权、密钥、challenge、签名和撤销流程 |

当前方案优先考虑服务商改动小、部署和维护成本低。Karing/Clash Mi 后续也许会支持安全能力更完整的 `OAuth + PKCE + Ed25519` 方案；但相应地，服务商需要实现授权端点、客户端注册与回调、设备密钥绑定、challenge、签名校验和撤销等能力。
